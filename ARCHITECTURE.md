...
- 2026-07-19 · (architect) **Design v3 "מקצב ונשימה" executed by the architect** (10 commits, DESIGN §4a + §5 amended in both copies): site-wide band rhythm (paper ↔ pine-soft) with `SectionCurve` arc dividers; `--pine-deep` anchor footer with the sleeping mascot (replaces the thin one-line footer); paper-grain texture on body; sparse-content `JoinCard` appended to unfiltered cat grids while results < 8 (launch-phase rule); both wizards got a named `WizardSteps` indicator with the Peeking Cat on the current step (the upload wizard previously had NO indicator), cat-name-personalized step headings, honest `SavedNote` save confirmations, and segmented controls for sex + yes/no questions (same form semantics, radios underneath); cat detail page: sticky in-flow mobile CTA bar (releases before the footer, clears the support launcher) + neutral chip fill unified; about page rebuilt as banded storytelling with bento fact cards; special-needs filter highlighted in marmalade with a brand Heart icon replacing the 💙 emoji everywhere (anti-pattern fix); password-field show-toggle overlap fixed via logical padding inside the input's LTR context. All gates green (conventions 0, tsc 0, lint 0/0, 43 vitest, prod build); E2E-verified in browser incl. full wizard walk as publisher1 (draft created then deleted, seed data intact).
