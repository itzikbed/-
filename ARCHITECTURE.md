...
- 2026-07-16 · Account deletion cascade confirmed: cats.owner_id is ON DELETE CASCADE in migration 0001 — archive-before-delete is dead code for already-deleted users. The implementation uses a full cascade: moderation_log entry first (action='account_deletion'), then auth.admin.deleteUser() which cascades to cats, adoption_requests, adopter_profiles. email_log.recipient_user_id is ON DELETE SET NULL (migration 0012), preserving audit records. moderation_log rows survive deletion (no FK to auth.users).
...
- 2026-07-16 · Hero film: replaced hero_2 (over-compressed wide shot failing mobile legibility bar) with Mixkit close-up cat clip. License doc created at docs/hero-media-licenses.md. All clips now meet DESIGN.md §6b.3 criteria: close-up/medium shots, cat fills ≥40% of 390px frame.
