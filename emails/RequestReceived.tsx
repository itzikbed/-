...
export const getSubject = (catName?: string, catSex: 'male' | 'female' | 'unknown' = 'unknown') => {
  ...
  return gendered('emails', 'requestReceivedSubject', catSex).replace('{name}', name)
}

export default function RequestReceived({ catName = '', catSex = 'unknown' }: RequestReceivedProps) {
  const requestsUrl = `${process.env.NEXT_PUBLIC_SITE_URL || 'http://localhost:3000'}/requests`
  const name = catName || strings.common.defaultCatName
  
  const heading = gendered('emails', 'requestReceivedHeading', catSex).replace('{name}', name)
  const bodyText = gendered('emails', 'requestReceivedText', catSex).replace('{name}', name)
    ...
    <Html lang="he" dir="rtl">
      ...
      <Body style={mainStyle} dir="rtl">
        <Container style={containerStyle} dir="rtl">
          ...
          <MsoButton href={requestsUrl} text={gendered('emails', 'requestReceivedCta', catSex)} />
