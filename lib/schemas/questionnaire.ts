...
export const questionnaireBaseSchema = z.object({
  ...
  age: z.number()
    ...
    .min(18, { message: 'הגיל חייב להיות לפחות 18' })
  ...
  // Step 2: Home
  has_other_pets: z.boolean({ message: 'נא לבחור אחת מהאפשרויות' }),
  other_pets_desc: z.string().max(1000).optional().nullable(),
  has_cat_experience: z.boolean({ message: 'נא לבחור אחת מהאפשרויות' }),
  floor_type: z.enum(['ground_house', 'garden_floor', 'floor_1', 'floor_2', 'floor_3_plus'], { message: 'נא לבחור סוג קומה' }),
  has_window_screens: z.boolean({ message: 'נא לבחור אחת מהאפשרויות' }),
